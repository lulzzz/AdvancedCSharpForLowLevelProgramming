...
\begin{enumerate}
	...
	\item Struct B have Size specified to be smaller than the actual size of Struct, therefore the size parameter is ignored for this layout, the size for this struct become 2 bytes. It also isn't padded to the multiple of 4 or 8, because 2 bytes can be aligned in a single 4 bytes word, you could even have 3 byte members and the size of struct would become 3 bytes long.
